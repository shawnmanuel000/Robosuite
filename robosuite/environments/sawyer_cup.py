...
from robosuite.models.objects import CupObject, StandObject
...
class SawyerCup(SawyerEnv):
    ...
    def _load_model(self):
        ...
        cup = CupObject()
        stand = StandObject()
        self.mujoco_objects = OrderedDict([("cup", cup), ("stand", stand)])
    ...
    def _get_reference(self):
        ...
        self.handle_body_id = self.sim.model.body_name2id("handle_1")
        self.cup_body_id = self.sim.model.body_name2id("cup")
        ...
        self.stand_corner_id = self.sim.model.site_name2id("stand_corner")
    ...
    def _reset_internal(self):
        ...
        self.cup_init_pos = self.sim.data.body_xpos[self.cup_body_id].copy()[2]
    ...
    def reward(self, action=None):
        ...
        if self.reward_shaping:
            ...
            # reaching reward
            handle_pos = self.sim.data.site_xpos[self.handle_site_id]
            ...
            if touch_left_finger or touch_right_finger:
                ...
                reward += 0.25
                dist = np.linalg.norm(handle_pos - stand_pos)
    ...
    def _object_obs(self):
        di = {}
        cup_pos = np.array(self.sim.data.body_xpos[self.cup_body_id])
        cup_quat = convert_quat(
            np.array(self.sim.data.body_xquat[self.cup_body_id]), to="xyzw"
        )
        handle_site = np.array(self.sim.data.site_xpos[self.handle_site_id])
        cup_site = np.array(self.sim.data.site_xpos[self.cup_site_id])
        stand_end_site = np.array(self.sim.data.site_xpos[self.stand_end_id])
        stand_corner_site = np.array(self.sim.data.site_xpos[self.stand_corner_id])
        gripper_site_pos = np.array(self.sim.data.site_xpos[self.eef_site_id])


        di["cup_pos"] = cup_pos
        di["cup_quat"] = cup_quat
        di["handle_vec"] = (handle_site - cup_site)[:2]
        di["cup_to_stand"] = cup_site - stand_end_site
        di["handle_to_stand"] = handle_site - stand_end_site

        di["gripper_to_cup"] = gripper_site_pos - cup_pos
        di["handle_to_stand"] = handle_site - stand_end_site
        di["stand_corner_to_end"] = stand_corner_site - stand_end_site
    ...
    def _get_observation(self):
        ...
        if self.use_object_obs:
            ...
            obj_obs = self._object_obs()

            di.update(obj_obs)
            di["object-state"] = np.concatenate(
                [obj_obs[k] for k in obj_obs]
            )
    ...
    def _check_success(self):
        ...
        """
        cup_pos = self.sim.data.site_xpos[self.handle_site_id]
        stand_pos = self.sim.data.site_xpos[self.stand_corner_id]

        # cube is higher than the table top above a margin
        return np.linalg.norm(cup_pos - stand_pos) < 0.005
